...
function screen_content()
{
	...
	print "<h3>Bulk Import of Job Status Codes from CSV</h3>";
...
function print_form()
{
	...
	You can upload a CSV file of VILNOs and Status Codes (for Collection jobs only).<br>
	The system will set the Job Status on each job found in the CSV to the new Status Code for that job.<br>
	The columns of the CSV file should be:
	<ul style=\"margin-top:0px;\">
		<li>VIL Number</li>
		<li>Status Code (e.g. RCR)</li>
	</ul>
	(if any columns are found after the Status Code column, they will be ignored)
	</div><!--div_guide-->
	";
...
function import_csv($file)
{
	...
	while (($line = fgetcsv($fhan)) !== false)
	{
		...
		$rc = status_change_wrapper($line, $lix);
		if ($rc)
...
function status_change_wrapper($line, $lix)
{
	global $job_id;
	global $jobs_list;
	global $sqlFalse;
	...
	$feedback_188 = (global_debug() ? true : false);#REVIEW: very old code waiting to go live
	...
	{
		list($ms_top, $my_limit) = sql_top_limit(1);
		$sql = "SELECT $ms_top JOB_ID, JC_JOB, JOB_CLOSED FROM JOB WHERE (J_VILNO=$vilno) AND (OBSOLETE=$sqlFalse) ORDER BY JOB_ID DESC $my_limit";
	}
	sql_execute($sql);
	$job_id = 0;
	$jc_job = 0;
	...
	if ($job_closed)
		return "The job with VILNo \"{$line[$fix]}\" is closed and so will not be updated";
	$fix++;

	dprint("Found JOB_ID $job_id");

	$new_code = trim($line[$fix]);
	if (!$new_code)
		return "The new Status Code is empty (for job with VILNo \"$vilno\")";
	$sql = "SELECT JOB_STATUS_ID, J_STTS_CLOSED FROM JOB_STATUS_SD WHERE J_STATUS='$new_code' AND OBSOLETE=$sqlFalse";
	sql_execute($sql);
	$job_status_id = 0;
	$j_stts_closed = 0;
	while (($newArray = sql_fetch()) != false)
	{
		$job_status_id = intval($newArray[0]);
		$j_stts_closed = intval($newArray[1]);
	}
	if (!$job_status_id)
		return "The new Status Code ($new_code) was not recognised (for job with VILNo \"$vilno\")";

	# Update the job
	$sql = "UPDATE JOB SET JC_JOB_STATUS_ID=$job_status_id WHERE JOB_ID=$job_id";
	audit_setup_job($job_id, 'JOB', 'JOB_ID', $job_id, 'JC_JOB_STATUS_ID', $job_status_id);
	sql_execute($sql, true); # audited
	if ($j_stts_closed)
		sql_close_job($job_id, false);
	sql_update_job($job_id);

	$jobs_list[] = array('LNO' => $lix, 'VILNO' => $vilno, 'NEW_CODE' => $new_code, 'CLOSED' => $j_stts_closed, 'CODE_ID' => $job_status_id, 'JOB_ID' => $job_id);

	return ''; # success

} # status_change_wrapper()
...
function print_jobs()
{
	...
	print "
	<h3 style=\"color:blue;\">The following jobs have been processed:</h3>
	<table class=\"spaced_table\">
	<tr>
		<th>CSV Line</th><th>VILNo</th><th>New Code</th><th>Closed</th><th>Code ID</th><th>Job ID</th>
	</tr>
	";
	foreach ($jobs_list as $one)
	{
		...
		<tr>
			<td $ar>{$one['LNO']}</td>
			<td $ar>{$one['VILNO']}</td>
			<td $ar>{$one['NEW_CODE']}</td>
			<td $ar>{$one['CLOSED']}</td>
			<td $ar>{$one['CODE_ID']}</td>
			<td $ar>{$one['JOB_ID']}</td>
		</tr>
		";
	}
