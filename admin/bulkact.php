...
if ($USER['IS_ENABLED'])
{
	...
	$page_title_2 = 'Bulk Activity - Vilcol';
	screen_layout();
}
else
...
function screen_content()
{
	...
	print "<h3>Bulk Import of Job Activity Codes from CSV</h3>";
...
function print_form()
{
	...
	You can upload a CSV file of job activity codes (for Collection jobs only).<br>
	The respective jobs will then have the activity code added to them.<br>
	The columns of the CSV file should be:
	<ul style=\"margin-top:0px;\">
		<li>VIL Number</li>
		<li>Activity Code (e.g. EML)</li>
	</ul>
	</div><!--div_guide-->
	";
...
function activity_add($line, $lix)
{
	global $jobs;
	global $job_id;
	global $sqlFalse;
	...
	$act_code = strtoupper(trim($field_B));
	...
	if ($act_code)
	{
		...
		if (!$activity_id)
			return "No ACTIVITY_SD record found from activity code $act_code (from \"$field_B\")";
	}
	else
		return "No activity code (from \"$field_B\")";

	dprint("Found JOB_ID $job_id, activity ID $activity_id)");

	sql_add_activity($job_id, 1, $activity_id, false);
	sql_add_note($job_id, "Activity code $act_code added from CSV import", false);
	sql_update_job($job_id);

	$jobs[] = array('LNO' => $lix, 'VILNO' => $vilno, 'JOB_ID' => $job_id,
...
function print_jobs()
{
	...
	<table class=\"spaced_table\">
	<tr>
		<th>CSV Line</th><th>VILNo</th><th>Activity Added</th><th>Job ID</th><th>Activity ID</th>
	</tr>
	";
	foreach ($jobs as $one)
	{
