...
if ($USER['IS_ENABLED'])
{
	...
	$page_title_2 = 'Bulk Addresses - Vilcol';
	screen_layout();
}
else
...
function screen_content()
{
	...
	print "<h3>Bulk Import of Subject Addresses from CSV</h3>";
...
function print_form()
{
	...
	You can upload a CSV file of subject addresses (for Collection jobs only).<br>
	The addresses will then be automatically added to the respective jobs.<br>
...
function address_add($line, $lix)
{
	...
	$addr = array();
	$fix_max = count($line) - 1;
	for ($ii = 0; $ii < 6; $ii++) # ADDR_1 to 5 and PC
	{
		if ($fix <= $fix_max)
			$temp = trim($line[$fix++]);
		else
	...
	#dprint("addr/1=" . print_r($addr,1));#
	$pcode = '';
	for ($ii = count($addr)-1; 0 <= $ii; $ii--)
	{
		if (is_postcode($addr[$ii]))
		{
			$pcode = $addr[$ii];
			unset($addr[$ii]);
			break;
	...
	for ($ii = count($addr); $ii < 5; $ii++)
		$addr[$ii] = '';
	#dprint("addr/2=" . print_r($addr,1));#
	if ((count($addr) == 0) && (!$pcode))
		return "No address found";

	# Get Primary Subject (or any subject if none are primary)
	$sql = "SELECT JOB_SUBJECT_ID FROM JOB_SUBJECT WHERE JOB_ID=$job_id AND OBSOLETE=$sqlFalse ORDER BY JS_PRIMARY DESC";
	sql_execute($sql);
	$subject_id = 0;
	while (($newArray = sql_fetch()) != false)
		$subject_id = $newArray[0];
	if (!$subject_id)
		return "The job with VILNo \"{$line[$fix]}\" (and ID $job_id) has no subjects";

	dprint("Found JOB_ID $job_id, Address " . print_r($addr,1) . ", Pcode \"$pcode\".");

	# Add address to job
	add_new_address($subject_id, $addr, $pcode);

	$addresses[] = array('LNO' => $lix, 'VILNO' => $vilno, 'JOB_ID' => $job_id, 'SUBJECT_ID' => $subject_id,
			'ADDR_1' => $addr[0], 'ADDR_2' => $addr[1], 'ADDR_3' => $addr[2], 'ADDR_4' => $addr[3], 'ADDR_5' => $addr[4], 'POSTCODE' => $pcode);
...
function print_addresses()
{
	...
	print "
	<h3 style=\"color:blue;\">The following addresses have been added to the database:</h3>
	<table class=\"spaced_table\">
	<tr>
		<th>CSV Line</th><th>VILNo</th><th>Address 1</th><th>Address 2</th><th>Address 3</th><th>Address 4</th><th>Address 5</th><th>Postcode</th><th>Job ID</th><th>Subject ID</th>
	</tr>
	";
