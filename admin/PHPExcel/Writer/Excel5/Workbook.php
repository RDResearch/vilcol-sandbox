...
class Workbook extends \PhpOffice\PhpSpreadsheet\Writer\Xls\BIFFwriter
{
  ...
  */
 public function _addFont(\PhpOffice\PhpSpreadsheet\Style\Font $font)
	{
	...
	private function _writeAllDefinedNamesBiff8()
	{
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isColumnsToRepeatAtLeftSet() && $sheetSetup->isRowsToRepeatAtTopSet()) {
				...
				$chunk .= $this->writeData($this->_writeDefinedNameBiff8(\pack('C', 0x07), $formulaData, $i + 1, \true));
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isPrintAreaSet()) {
				...
				// store the DEFINEDNAME record
				$chunk .= $this->writeData($this->_writeDefinedNameBiff8(\pack('C', 0x06), $formulaData, $i + 1, \true));
			}
		}
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if(!empty($autoFilterRange)) {
				...
				$chunk .= $this->writeData($this->_writeShortNameBiff8($name, $i + 1, $rangeBounds, \true));
			}
		}
 ...
 private function _writeBoundsheet($sheet, $offset)
	{
		...
		switch ($sheet->getSheetState()) {
			case \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::SHEETSTATE_VISIBLE:	$ss = 0x00; break;
			case \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::SHEETSTATE_HIDDEN:		$ss = 0x01; break;
			case \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::SHEETSTATE_VERYHIDDEN:	$ss = 0x02; break;
