...
class Worksheet extends \PhpOffice\PhpSpreadsheet\Writer\Xls\BIFFwriter
{
 ...
 private $_parser;
	 ...
	 * @var integer
	 */
	public $_xls_strmax = 255;

	/**
	 * Array containing format information for columns
	 * @var array
	 */
	public $_colinfo = array();

	/**
	 * Array containing the selected area for the worksheet
	 * @var array
	 */
	public $_selection = array(0,0,0,0);

	/**
	 * The active pane for the worksheet
	 * @var integer
	 */
	public $_active_pane = 3;

	/**
	 * Whether to use outline.
	 * @var integer
	 */
	public $_outline_on = 1;

	/**
	 * Auto outline styles.
	 * @var bool
	 */
	public $_outline_style = 0;

	/**
	 * Whether to have outline summary below.
	 * @var bool
	 */
	public $_outline_below = 1;

	/**
	 * Whether to have outline summary at the right.
	 * @var bool
	 */
	public $_outline_right = 1;
...
//		$this->_firstRowIndex = ($minR > 65535) ? 65535 : $minR;
		...
		$this->_firstColumnIndex	= \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($minC);
		$this->_lastColumnIndex		= \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($maxC);
...
//		if ($this->_firstColumnIndex > 255) $this->_firstColumnIndex = 255;
	...
	function close()
	{
		...
		if (($defaultWidth = $_phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
			$defaultWidth = \PhpOffice\PhpSpreadsheet\Shared\Font::getDefaultColumnWidthByFont($_phpSheet->getParent()->getDefaultStyle()->getFont());
		}
		...
		// Write Cells
		foreach ($_phpSheet->getCoordinates() as $cellID) {
			$cell = $_phpSheet->getCell($cellID, true);
			$row = $cell->getRow() - 1;
...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
		...
		foreach ($_phpSheet->getHyperLinkCollection() as $coordinate => $hyperlink) {
			...
			$this->_writeUrl($row - 1, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($column) - 1, $url);
		}
	...
	private function _writeBIFF8CellRangeAddressFixed($range = 'A1')
	{
		...
		return(\pack('vvvv',
			...
			\PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($firstCellCoordinates[0]) - 1,
			\PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($lastCellCoordinates[0]) - 1
		));
	}
	...
	private function _writeWindow2()
	{
		...
		$fPaged		 = 1;					 // 2
		$fPageBreakPreview = $this->_phpSheet->getSheetView()->getView() === \PhpOffice\PhpSpreadsheet\Worksheet\SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW;
	...
	private function _writeSelection()
	{
		...
		$selectedCells = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::splitRange($this->_phpSheet->getSelectedCells());
	...
	private function _writeSetup()
	{
		...
		// Page orientation
		$fLandscape = ($this->_phpSheet->getPageSetup()->getOrientation() == \PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::ORIENTATION_LANDSCAPE) ?
			0x0 : 0x1;
	...
	private function _writeHeader()
	{
		...
		*/
		...
		$recordData = \PhpOffice\PhpSpreadsheet\Shared\StringHelper::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddHeader());
		$length = \strlen($recordData);
	...
	private function _writeFooter()
	{
		...
		*/
		...
		$recordData = \PhpOffice\PhpSpreadsheet\Shared\StringHelper::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddFooter());
		$length = \strlen($recordData);
	...
	private function _writeAutoFilterInfo(){
		...
		$rangeBounds = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::rangeBoundaries($this->_phpSheet->getAutoFilter()->getRange());
	...
	function _positionImage($col_start, $row_start, $x1, $y1, $width, $height)
	{
		...
		if ($x1 >= \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->_phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_start))) {
			$x1 = 0;
		}
		if ($y1 >= \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeRow($this->_phpSheet, $row_start + 1)) {
			$y1 = 0;
		}
		...
		while ($width >= \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->_phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_end))) {
			$width -= \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->_phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_end));
			++$col_end;
		}

		// Subtract the underlying cell heights to find the end cell of the image
		while ($height >= \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeRow($this->_phpSheet, $row_end + 1)) {
			$height -= \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeRow($this->_phpSheet, $row_end + 1);
			++$row_end;
		}
		...
		//
		if (\PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->_phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_start)) == 0) {
			return;
		}
		if (\PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->_phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_end))   == 0) {
			return;
		}
		if (\PhpOffice\PhpSpreadsheet\Shared\Xls::sizeRow($this->_phpSheet, $row_start + 1) == 0) {
			return;
		}
		if (\PhpOffice\PhpSpreadsheet\Shared\Xls::sizeRow($this->_phpSheet, $row_end + 1)   == 0) {
			return;
		}

		// Convert the pixel values to the percentage value expected by Excel
		$x1 = $x1	 / \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->_phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_start))   * 1024;
		$y1 = $y1	 / \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeRow($this->_phpSheet, $row_start + 1)   *  256;
		$x2 = $width  / \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->_phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_end))	 * 1024; // Distance to right side of object
		$y2 = $height / \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeRow($this->_phpSheet, $row_end + 1)	 *  256; // Distance to bottom of object
 ...
 public function setEscher(\PhpOffice\PhpSpreadsheet\Shared\Escher $pValue = \null)
	...
	private function _writeMsoDrawing()
	{
		...
		if (isset($this->_escher)) {
			$writer = new \PhpOffice\PhpSpreadsheet\Writer\Xls\Escher($this->_escher);
			$data = $writer->close();
	...
	private function _writeDataValidity()
	{
		...
		if (!empty($dataValidationCollection)) {
			...
			foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
				...
				switch ($type) {
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::TYPE_NONE:		$type = 0x00;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::TYPE_WHOLE:		$type = 0x01;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::TYPE_DECIMAL:	$type = 0x02;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::TYPE_LIST:		$type = 0x03;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::TYPE_DATE:		$type = 0x04;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::TYPE_TIME:		$type = 0x05;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::TYPE_TEXTLENGTH:	$type = 0x06;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::TYPE_CUSTOM:		$type = 0x07;	break;
				}
				$options |= $type << 0;
				...
				switch ($errorStyle) {
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::STYLE_STOP:			$errorStyle = 0x00;		break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::STYLE_WARNING:		$errorStyle = 0x01;		break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::STYLE_INFORMATION:	$errorStyle = 0x02;		break;
				}
				...
				switch ($operator) {
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::OPERATOR_BETWEEN: $operator = 0x00			;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::OPERATOR_NOTBETWEEN: $operator = 0x01		;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::OPERATOR_EQUAL: $operator = 0x02				;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::OPERATOR_NOTEQUAL: $operator = 0x03			;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::OPERATOR_GREATERTHAN: $operator = 0x04		;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::OPERATOR_LESSTHAN: $operator = 0x05			;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::OPERATOR_GREATERTHANOREQUAL: $operator = 0x06;	break;
					case \PhpOffice\PhpSpreadsheet\Cell\DataValidation::OPERATOR_LESSTHANOREQUAL: $operator = 0x07	;	break;
				}
				$options |= $operator << 20;
  ...
  */
 private function _writeCFRule(\PhpOffice\PhpSpreadsheet\Style\Conditional $conditional){
		...
		if($conditional->getConditionType() == \PhpOffice\PhpSpreadsheet\Style\Conditional::CONDITION_EXPRESSION){
			$type = 0x02;
			$operatorType = 0x00;
		} else if($conditional->getConditionType() == \PhpOffice\PhpSpreadsheet\Style\Conditional::CONDITION_CELLIS){
			$type = 0x01;
		...
		if($numConditions == 1){
			...
			$operand2 = \null;
		} else if($numConditions == 2 && ($conditional->getOperatorType() == \PhpOffice\PhpSpreadsheet\Style\Conditional::OPERATOR_BETWEEN)){
			$szValue1 = ($arrConditions[0] <= 65535 ? 3 : 0x0000);
		...
		if($bFormatFont == 1){
			...
			if($conditional->getStyle()->getFont()->getSubScript() == \true){
				...
				$fontEscapement = 0;
			} else if($conditional->getStyle()->getFont()->getSuperScript() == \true){
			...
			switch ($conditional->getStyle()->getFont()->getUnderline()){
				case \PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_NONE             : $dataBlockFont .= \pack('C', 0x00); $fontUnderline = 0; break;
				case \PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_DOUBLE           : $dataBlockFont .= \pack('C', 0x02); $fontUnderline = 0; break;
				case \PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_DOUBLEACCOUNTING : $dataBlockFont .= \pack('C', 0x22); $fontUnderline = 0; break;
				case \PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLE           : $dataBlockFont .= \pack('C', 0x01); $fontUnderline = 0; break;
				case \PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLEACCOUNTING : $dataBlockFont .= \pack('C', 0x21); $fontUnderline = 0; break;
		...
		if($bFormatAlign == 1){
			...
			switch ($conditional->getStyle()->getAlignment()->getHorizontal()){
				case \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_GENERAL 			: $blockAlign = 0; break;
				case \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT				: $blockAlign = 1; break;
				case \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT				: $blockAlign = 3; break;
				case \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER			: $blockAlign = 2; break;
				case \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER_CONTINUOUS	: $blockAlign = 6; break;
				case \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_JUSTIFY			: $blockAlign = 5; break;
			}
			...
			switch ($conditional->getStyle()->getAlignment()->getVertical()){
				case \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_BOTTOM 				: $blockAlign = 2 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_TOP					: $blockAlign = 0 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER				: $blockAlign = 1 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_JUSTIFY				: $blockAlign = 3 << 4; break;
			}
			$blockAlign |= 0 << 7;
		...
		if($bFormatBorder == 1){
			...
			switch ($conditional->getStyle()->getBorders()->getLeft()->getBorderStyle()){
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE              : $blockLineStyle |= 0x00; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN              : $blockLineStyle |= 0x01; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUM            : $blockLineStyle |= 0x02; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHED            : $blockLineStyle |= 0x03; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOTTED            : $blockLineStyle |= 0x04; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK             : $blockLineStyle |= 0x05; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOUBLE            : $blockLineStyle |= 0x06; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_HAIR              : $blockLineStyle |= 0x07; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHDOT           : $blockLineStyle |= 0x09; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D; break;
			}
			switch ($conditional->getStyle()->getBorders()->getRight()->getBorderStyle()){
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE              : $blockLineStyle |= 0x00 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN              : $blockLineStyle |= 0x01 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUM            : $blockLineStyle |= 0x02 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHED            : $blockLineStyle |= 0x03 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOTTED            : $blockLineStyle |= 0x04 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK             : $blockLineStyle |= 0x05 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOUBLE            : $blockLineStyle |= 0x06 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_HAIR              : $blockLineStyle |= 0x07 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHDOT           : $blockLineStyle |= 0x09 << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C << 4; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D << 4; break;
			}
			switch ($conditional->getStyle()->getBorders()->getTop()->getBorderStyle()){
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE              : $blockLineStyle |= 0x00 << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN              : $blockLineStyle |= 0x01 << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUM            : $blockLineStyle |= 0x02 << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHED            : $blockLineStyle |= 0x03 << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOTTED            : $blockLineStyle |= 0x04 << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK             : $blockLineStyle |= 0x05 << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOUBLE            : $blockLineStyle |= 0x06 << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_HAIR              : $blockLineStyle |= 0x07 << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08 << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHDOT           : $blockLineStyle |= 0x09 << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C << 8; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D << 8; break;
			}
			switch ($conditional->getStyle()->getBorders()->getBottom()->getBorderStyle()){
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE              : $blockLineStyle |= 0x00 << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN              : $blockLineStyle |= 0x01 << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUM            : $blockLineStyle |= 0x02 << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHED            : $blockLineStyle |= 0x03 << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOTTED            : $blockLineStyle |= 0x04 << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK             : $blockLineStyle |= 0x05 << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOUBLE            : $blockLineStyle |= 0x06 << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_HAIR              : $blockLineStyle |= 0x07 << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHED      : $blockLineStyle |= 0x08 << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHDOT           : $blockLineStyle |= 0x09 << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHDOT     : $blockLineStyle |= 0x0A << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHDOTDOT        : $blockLineStyle |= 0x0B << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHDOTDOT  : $blockLineStyle |= 0x0C << 12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_SLANTDASHDOT      : $blockLineStyle |= 0x0D << 12; break;
			}
			...
			switch ($conditional->getStyle()->getBorders()->getDiagonal()->getBorderStyle()){
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE              : $blockColor |= 0x00 << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN              : $blockColor |= 0x01 << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUM            : $blockColor |= 0x02 << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHED            : $blockColor |= 0x03 << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOTTED            : $blockColor |= 0x04 << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK             : $blockColor |= 0x05 << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOUBLE            : $blockColor |= 0x06 << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_HAIR              : $blockColor |= 0x07 << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHED      : $blockColor |= 0x08 << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHDOT           : $blockColor |= 0x09 << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHDOT     : $blockColor |= 0x0A << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DASHDOTDOT        : $blockColor |= 0x0B << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUMDASHDOTDOT  : $blockColor |= 0x0C << 21; break;
				case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_SLANTDASHDOT      : $blockColor |= 0x0D << 21; break;
			}
		...
		if($bFormatFill == 1){
			...
			switch ($conditional->getStyle()->getFill()->getFillType()){
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_NONE						: $blockFillPatternStyle = 0x00; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID					: $blockFillPatternStyle = 0x01; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_MEDIUMGRAY		: $blockFillPatternStyle = 0x02; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_DARKGRAY			: $blockFillPatternStyle = 0x03; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_LIGHTGRAY		: $blockFillPatternStyle = 0x04; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_DARKHORIZONTAL	: $blockFillPatternStyle = 0x05; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_DARKVERTICAL		: $blockFillPatternStyle = 0x06; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_DARKDOWN			: $blockFillPatternStyle = 0x07; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_DARKUP			: $blockFillPatternStyle = 0x08; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_DARKGRID			: $blockFillPatternStyle = 0x09; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_DARKTRELLIS		: $blockFillPatternStyle = 0x0A; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_LIGHTHORIZONTAL	: $blockFillPatternStyle = 0x0B; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_LIGHTVERTICAL	: $blockFillPatternStyle = 0x0C; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_LIGHTDOWN		: $blockFillPatternStyle = 0x0D; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_LIGHTUP			: $blockFillPatternStyle = 0x0E; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_LIGHTGRID		: $blockFillPatternStyle = 0x0F; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_LIGHTTRELLIS		: $blockFillPatternStyle = 0x10; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_GRAY125			: $blockFillPatternStyle = 0x11; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_GRAY0625			: $blockFillPatternStyle = 0x12; break;
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_GRADIENT_LINEAR			: $blockFillPatternStyle = 0x00; break;	// does not exist in BIFF8
				case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_GRADIENT_PATH			: $blockFillPatternStyle = 0x00; break;	// does not exist in BIFF8
