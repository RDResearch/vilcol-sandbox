...
if ($USER['IS_ENABLED'])
{
	...
	$page_title_2 = 'Bulk Notes - Vilcol';
	screen_layout();
}
else
...
function screen_content()
{
	...
	print "<h3>Bulk Import of Notes from CSV</h3>";
...
function print_form()
{
	...
	You can upload a CSV file of notes on jobs.<br>
...
function note_add($line, $lix)
{
	...
	$sql = "SELECT $ms_top JOB_ID FROM JOB WHERE (J_VILNO=$vilno) AND (OBSOLETE=$sqlFalse) ORDER BY JOB_ID DESC $my_limit";
	sql_execute($sql);
	$job_id = 0;
	...
	$note = $line[$fix];
	if ((!$note) || ($note == 'NULL'))
		return "The note \"{$line[$fix]}\" is invalid";
	$fix++;

	dprint("Found JOB_ID $job_id, Note \"$note\"");

	# Add note to job
	$job_note_id = sql_add_note($job_id, $note);

	$notes[] = array('LNO' => $lix, 'VILNO' => $vilno, 'JOB_ID' => $job_id, 'NOTE' => $note, 'JOB_NOTE_ID' => $job_note_id);
	return ''; # success
...
function print_notes()
{
	...
	<table class=\"spaced_table\">
	<tr>
		<th>CSV Line</th><th>VILNo</th><th>Note</th><th>Job ID</th><th>Job Note ID</th>
	</tr>
	";
